...
// 2026-09-16'da Aladdin'e (Fatih Gezgin'in mevcut hesabından AYRI, ikinci bir Paraşüt hesabı)
// günlük iç fatura kesmek için ihtiyaç doğunca, tek-hesaplı istemci bir FABRİKAYA dönüştürüldü —
// her hesap kendi token önbelleğini/single-flight kilidini/http istemcisini alır, ikisi birbirine
// KARIŞMAZ. Aşağıdaki `parasutGet`/`parasutPost`/vb. export'ları mevcut (Fatih Gezgin) hesap için
// bu fabrikanın BİREBİR AYNI davranışta bir örneğidir — hiçbir mevcut çağıran (contacts.ts,
// invoices.ts, eArchives.ts, products.ts, orderInvoice.ts) DEĞİŞMEDİ. Config, ESKİDEN OLDUĞU GİBİ
// (bir kerede modül yüklenirken değil) her gerçek istekte LAZY olarak okunuyor — bu, env
// değişkenleri eksik olsa bile modülün import edilebilmesini sağlıyor (ör. build/test ortamları).
export function createParasutClient(getConfig: () => ParasutAccountConfig) {
  ...
  // (bkz. app/orders/InvoiceAndAseButton.tsx) — token süresi dolduğunda/boşken bunların hepsi AYNI
  ...
  function createHttpClient(): AxiosInstance {
    // BİLEREK burada `baseURL` VERİLMİYOR — axios.create() SENKRON ve MODÜL YÜKLENİRKEN
    // çalışıyor (aşağıdaki `const http = createHttpClient();`), `getConfig()` ise eksik bir env
    // değişkeninde HATA FIRLATIYOR. baseURL'i burada okusaydık, PARASUT_*/PARASUT2_* env
    // değişkenleri eksikken bu modülü sadece IMPORT ETMEK bile uygulamayı çökertirdi — orijinal
    // (tek hesaplı) istemcinin "sadece gerçekten kullanıldığı yerde anlamlı hata verir" garantisini
    // bozardı (2026-09-16'da bu refactor sırasında fark edilip düzeltildi). Bunun yerine tam URL
    // her istekte parasutRequest içinde (zaten lazy olan getConfig() ile) kuruluyor.
    ...
    while (true) {
      try {
        ...
        const response = await http.request<T>({
          method,
          url: `${config.baseUrl}/v4/${config.companyId}/${path}`,
          data: body,
  ...
  return {
    parasutGet: <T>(path: string): Promise<T> => parasutRequest<T>("GET", path),
    parasutPost: <T>(path: string, body: unknown): Promise<T> => parasutRequest<T>("POST", path, body),
    parasutPut: <T>(path: string, body: unknown): Promise<T> => parasutRequest<T>("PUT", path, body),
    parasutDelete: <T>(path: string): Promise<T> => parasutRequest<T>("DELETE", path),
  };
}

function requireParasutConfig(): ParasutAccountConfig {
  const missing = (
    [
  ...
  return {
    ...
    redirectUri: env.parasutRedirectUri,
    baseUrl: env.parasutBaseUrl,
  };
}

// Mevcut (Fatih Gezgin) hesap — Ozon müşterilerine kesilen satış faturaları için, ŞİMDİYE KADAR
// OLDUĞU GİBİ. Tüm mevcut çağıranlar (contacts.ts, invoices.ts, eArchives.ts, products.ts) bu
// export'ları değişmeden kullanmaya devam ediyor.
const defaultClient = createParasutClient(requireParasutConfig);

export const parasutGet = defaultClient.parasutGet;
export const parasutPost = defaultClient.parasutPost;
export const parasutPut = defaultClient.parasutPut;
export const parasutDelete = defaultClient.parasutDelete;
